...
# WHY WOULD YOU HARD CODE ANY OF THIS UGH
# app_id = "<FILL IN>"
    ...
    while success is False:
        try:
            ...
            if response.getcode() == 200:
                success = True 
        # TODO: stop mining at 400 errors!!
...
def processFacebookPageFeedStatus(status):
    ...
    status_message = '' if 'message' not in status else \
        unicode_decode(status['message']).replace('\n', '')
...
def scrapeFacebookPageFeedStatus(group_id, access_token, since_date, until_date, out_file_name=None):
    if(out_file_name is None):
        out_file_name = '{}_facebook_statuses.csv'.format(group_id)
    delim = '\t'
    with open(out_file_name, 'w') as file:
        w = csv.writer(file, delimiter=delim)
        w.writerow(["status_id", "status_message", "status_author", "status_author_id", "link_name",
