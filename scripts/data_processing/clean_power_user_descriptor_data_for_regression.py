"""
Clean power user anchor data for regression:
...
def main():
    ...
    parser.add_argument('--power_user_data_file', default='../../data/mined_tweets/combined_data_power_user_NE_flat_parsed_anchor.gz') # processed with anchors
    parser.add_argument('--power_user_engagement_file', default='../../data/mined_tweets/combined_data_power_user.gz') # raw data with engagement
    parser.add_argument('--clean_txt_file', default='../../data/mined_tweets/combined_data_power_user_tweet_clean_status.gz') # from extract_clean_status_txt_from_raw_data.py
    ...
    logging_file = '../../output/clean_power_user_anchor_data_for_regression.txt'
    ...
    # power user data
    power_user_data = pd.read_csv(args['power_user_data_file'], sep='\t', index_col=False, compression='gzip', converters={'date' : dateutil.parser.parse})
    # add engagement data
    power_user_engagement_data = pd.read_csv(args['power_user_engagement_file'], sep='\t', index_col=False, compression='gzip')
    id_var = 'id'
    engagement_vars = ['favorites', 'retweets']
    power_user_data = pd.merge(power_user_data, power_user_engagement_data.loc[:, [id_var]+engagement_vars], on=id_var, how='inner')
    ...
    power_user_data = power_user_data.assign(**{date_var : power_user_data.loc[:, date_var].apply(lambda x: fix_timezone(x))})
    # add time var
    ...
    power_user_data = power_user_data.assign(**{round_date_var : power_user_data.loc[:, date_var].apply(lambda x: round_to_day(x))})    
...
#     count_var = 'NE_freq'
    ...
    power_user_data_consistent = pd.merge(power_user_data, NE_count_peaks, on=[NE_var, data_name_var], how='inner')
    logging.debug('%d/%d consistent data'%(power_user_data_consistent.shape[0], power_user_data.shape[0]))
    
    ## assign peak times
    power_user_data_consistent = assign_relative_peak_time_vars(power_user_data_consistent, args['peak_date_buffer'])
    ...
    power_user_data_consistent_time_periods = []
    for author_i, data_i in power_user_data_consistent.groupby(author_var):
        time_periods_i = data_i.loc[:, time_period_vars].max(axis=0).sum()
        if(time_periods_i == len(time_period_vars)):
            power_user_data_consistent_time_periods.append(data_i)
        else:
            logging.debug('removed %d data from author %s with %d time periods'%(data_i.shape[0], author_i, time_periods_i))
    power_user_data_consistent_time_periods = pd.concat(power_user_data_consistent_time_periods, axis=0)
    logging.debug('after removing authors with inconsistent time periods: %d/%d authors, %d/%d data retained'%
                  (power_user_data_consistent_time_periods.loc[:, author_var].nunique(), power_user_data_consistent.loc[:, author_var].nunique(), power_user_data_consistent_time_periods.shape[0], power_user_data_consistent.shape[0]))
    # per-dataset
    data_name_var = 'data_name_fixed'
    logging.debug('after removing authors with inconsistent time periods: per-data author counts\n%s'%
                  (power_user_data_consistent_time_periods.groupby(data_name_var).apply(lambda x: x.loc[:, author_var].nunique())))
    power_user_data_consistent = power_user_data_consistent_time_periods.copy()
    ...
    power_user_data_consistent = pd.merge(power_user_data_consistent, anchor_data_prior_freq.loc[:, [NE_var, data_name_var, round_date_var, prior_NE_count_var]], on=[NE_var, data_name_var, round_date_var], how='inner')
    logging.debug('%d data with prior freq'%(power_user_data_consistent.shape[0]))
    logging.debug('prior freq data cols %s'%(','.join(sorted(power_user_data_consistent.columns))))
    ...
    author_post_counts = power_user_data_consistent.groupby([data_name_var, author_var]).apply(lambda x: get_roll_sum(x, round_date_var)).reset_index(name=post_count_var).rename(columns={'level_2':round_date_var})
    # NE counts
    NE_count_var = 'NE_count'
    author_NE_counts = power_user_data_consistent.groupby([data_name_var, author_var, NE_var]).apply(lambda x: get_roll_sum(x, round_date_var)).reset_index(name=NE_count_var).rename(columns={'level_3':round_date_var})
    ...
    for engagement_var in engagement_vars:
        ...
        engagement_counts = power_user_data_consistent.groupby([data_name_var, author_var, round_date_var]).apply(lambda x: x.loc[:, engagement_var].mean()).reset_index(name=prior_engagement_var)
        engagement_counts_change = power_user_data_consistent.groupby([data_name_var, author_var]).apply(lambda x: compute_data_change(x, engagement_var, date_var=round_date_var, period_shift=engagement_change_shift, data_cols=[data_name_var, author_var]))
    ...
    power_user_data_with_prior_stats = pd.merge(power_user_data_consistent, author_post_counts, on=[data_name_var, author_var, round_date_var], how='inner')
    power_user_data_with_prior_stats = pd.merge(power_user_data_with_prior_stats, author_NE_counts, on=[data_name_var, author_var, round_date_var, NE_var], how='inner')
    power_user_data_with_prior_stats = pd.merge(power_user_data_with_prior_stats, engagement_data, on=[data_name_var, author_var, round_date_var], how='inner')
    # add engagement change without shifting
    power_user_data_with_prior_stats = pd.merge(power_user_data_with_prior_stats, engagement_data_change, on=[data_name_var, author_var, round_date_var], how='left')
    # fix NA values for engagement change vars
    power_user_data_with_prior_stats.fillna(value={'%s_change'%(engagement_var) : 0. for engagement_var in engagement_vars}, inplace=True)
    # deduplicate
    power_user_data_with_prior_stats.drop_duplicates([id_var, NE_var], inplace=True)
    logging.debug('power user data has cols %s'%(','.join(sorted(power_user_data_with_prior_stats.columns))))
    logging.debug('power user data with prior stats\n%s'%(power_user_data_with_prior_stats.head()))
    # fix NA vals from engagement data merge
    combined_engagement_vars = ['prior_engagement', 'engagement_change']
    power_user_data_with_prior_stats.fillna(value={v : 0. for v in combined_engagement_vars}, inplace=True)
    
    ## add URL feature
    url_matcher = re.compile('<URL>')
    power_user_data_with_prior_stats = power_user_data_with_prior_stats.assign(**{'has_URL' : power_user_data_with_prior_stats.loc[:, 'txt'].apply(lambda x: int(url_matcher.search(x) is not None))})
    ...
    if(args.get('clean_txt_file') is not None):
        ...
        clean_txt_data = clean_txt_data[clean_txt_data.loc[:, id_var].isin(power_user_data_with_prior_stats.loc[:, id_var].unique())]
        power_user_data_with_prior_stats = pd.merge(power_user_data_with_prior_stats, clean_txt_data.loc[:, [id_var, image_video_url_var]], on=id_var, how='inner')
    
    ## add character length of post without context
    post_len_bins = 11
    power_user_data_with_prior_stats = compute_post_length(power_user_data_with_prior_stats, bins=post_len_bins)
    ...
    if(args['author_meta_file'] is not None):
        ...
        power_user_data_with_prior_stats = power_user_data_with_prior_stats.assign(**{author_var : power_user_data_with_prior_stats.loc[:, author_var].apply(lambda x: x.lower())})
        # merge
        logging.debug('metadata for %d authors'%(author_meta_data.shape[0]))
        meta_data_vars = ['organization', 'is_local']
        anchor_data_meta = pd.merge(power_user_data_with_prior_stats, author_meta_data.loc[:, [author_var, data_name_var] + meta_data_vars], on=[author_var, data_name_var], how='inner')
        logging.debug('%d/%d authors in combined meta_data+context_data'%(anchor_data_meta.loc[:, author_var].nunique(), power_user_data_with_prior_stats.loc[:, author_var].nunique()))
        power_user_data_with_prior_stats = anchor_data_meta.copy()
    
    ## cleaning spurious correlations: author/NE with 1 time period
    power_user_data_consistent_time_periods = []
    for author_i, data_i in power_user_data_with_prior_stats.groupby(author_var):
        time_periods_i = data_i.loc[:, time_period_vars].max(axis=0).sum()
#         if(time_periods_i == len(time_period_vars)):
        if(time_periods_i == 1):
            ...
            power_user_data_consistent_time_periods.append(data_i)
    power_user_data_with_prior_stats = pd.concat(power_user_data_consistent_time_periods, axis=0)
    power_user_data_consistent_time_periods = []
    for NE_i, data_i in power_user_data_with_prior_stats.groupby(NE_var):
        time_periods_i = data_i.loc[:, time_period_vars].max(axis=0).sum()
#         if(time_periods_i == len(time_period_vars)):
        if(time_periods_i == 1):
    ...
    power_user_data_consistent_author_NEs = []
    for NE_i, data_i in power_user_data_with_prior_stats.groupby(NE_var):
        if(data_i.loc[:, author_var].nunique() == 1):
            ...
            power_user_data_consistent_author_NEs.append(data_i)
    power_user_data_consistent_author_NEs = pd.concat(power_user_data_consistent_author_NEs, axis=0)
    power_user_data_consistent_authors = []
    for author_i, data_i in power_user_data_consistent_author_NEs.groupby(author_var):
        if(data_i.loc[:, NE_var].nunique() == 1):
            logging.debug('final cleaning: author %s has %d unique NEs'%(author_i, data_i.loc[:, NE_var].nunique()))
        else:
            power_user_data_consistent_authors.append(data_i)
    power_user_data_consistent_authors = pd.concat(power_user_data_consistent_authors, axis=0)
    power_user_data_with_prior_stats = power_user_data_consistent_authors.copy()
    ...
    for author_i, data_i in power_user_data_with_prior_stats.groupby(author_var):
        ...
        if(scalar_var_err <= err_min):
            ...
            power_user_data_scalar_clean.append(data_i)
    power_user_data_scalar_clean = pd.concat(power_user_data_scalar_clean, axis=0)
    power_user_data_with_prior_stats = power_user_data_scalar_clean.copy()
    
    ## save to file
    out_file = args['anchor_data_file'].replace('.gz', '_prior_author_stats.gz')
#     if(args['author_meta_file'] is not None):
#         out_file = args['anchor_data_file'].replace('.gz', '_prior_author_stats_with_meta.gz')
    if(not os.path.exists(out_file)):
        power_user_data_with_prior_stats.to_csv(out_file, sep='\t', index=False, compression='gzip')
