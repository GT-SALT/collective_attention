# run regression for active authors
DATA_FILE=../../data/mined_tweets/combined_tweet_tag_data_NE_flat_parsed_anchor_prior_author_stats.gz
OUT_DIR=../../output/active_authors/twitter_data/
if [ ! -d $OUT_DIR ];
then
    mkdir -p $OUT_DIR
fi
...
SCALAR_VARS=(NE_count_prior post_count NE_count prior_engagement engagement_change since_start)
